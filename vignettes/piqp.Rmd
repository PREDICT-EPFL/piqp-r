---
title: "PIQR Solver Example"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{PIQR Solver Example}
...
elements of $x_{lb}$ or $x_{ub}$ are $-\infty$ or $\infty$,
respectively. On the contrary, infinite values in the general
inequalities $Gx \leq h = \pm \infty$ can cause problems, i.e., they
are converted internally to `-1e30` and `1e30`, respectively.
...
The problem can now be solved via a call to `piqp()`.

```{r}
sol <- piqp(A, b, c, P, G, h, x_lb = x_lb, x_ub = x_ub)
cat(sprintf("(Solution status, description): = (%d, %s)\n",
            sol$status, status_description(sol$status)))
cat(sprintf("Objective: %f, solution: (x1, x2) = (%f, %f)\n", sol$pobj, sol$x[1], sol$x[2]))
```

`sol` contains several components, the most commonly used ones being:
...
- `run_time` : total runtime, if asked for in settings (see below).

Note how a descriptive string for the status was be obtained above using:
...
These interfaces can be requested explicitly via the `interface`
...
```{r}
sparse_sol <- piqp(A, b, c, P, G, h, x_lb = x_lb, x_ub = x_ub, interface = "sparse")
str(sparse_sol)
```
...
P <- as(as(P, "generalMatrix"), "CsparseMatrix")
c <- c(-1, -4)
A <- Matrix::Matrix(c(1, -2), ncol = 2, sparse = TRUE)
#A <- matrix(c(1, -2), ncol = 2)
b <- 0
x_lb <- rep(-1.0, 2)
x_ub <- rep(1.0, 2)
sol <- piqp(A = A, b = b, c = c, P = P, x_lb = x_lb, x_ub = x_ub)
cat(sprintf("(Solution status, description): = (%d, %s)\n",
            sol$status, status_description(sol$status)))
cat(sprintf("Objective: %f, solution: (x1, x2) = (%f, %f)\n", sol$pobj, sol$x[1], sol$x[2]))
```

But we can also choose to move the box constraints into the
inequalities and it should yield the same solution.

```{r}
G <- Matrix::Matrix(c(1, 0, -1, 0, 0, 1, 0, -1), byrow = TRUE,
                    nrow = 4, sparse = TRUE)
h <- rep(1, 4)
sol <- piqp(A = A, b = b, c = c, P = P, G = G, h = h)
cat(sprintf("(Solution status, description): = (%d, %s)\n",
            sol$status, status_description(sol$status)))
cat(sprintf("Objective: %f, solution: (x1, x2) = (%f, %f)\n", sol$pobj, sol$x[1], sol$x[2]))
```
...
verbosity, tolerances, etc.; see help on `piqp_control()`. As an
example, in the last problem, we can reduce the number of iterations.

```{r}
s <- piqp(A = A, b = b, c = c, P = P, G = G, h = h,
          control = list(max_iter = 3, verbose = TRUE)) ## Reduced number of iterations
cat(sprintf("(Solution status, description): = (%d, %s)\n",
            s$status, status_description(s$status)))
cat(sprintf("Objective: %f, solution: (x1, x2) = (%f, %f)\n", s$pobj, s$x[1], s$x[2]))
```
