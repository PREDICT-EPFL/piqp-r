...
s2 <- piqp(A1, b, c, P1, G1, h, x_lb = x_lb, x_ub = x_ub, interface = "sparse")
...
## CSC form
AA <- as(A, "CsparseMatrix")
PP <- as(P, "CsparseMatrix")
GG <- as(G, "CsparseMatrix")

s3 <- piqp(AA, b, c, PP, GG, h, x_lb = x_lb, x_ub = x_ub, interface = "sparse")
