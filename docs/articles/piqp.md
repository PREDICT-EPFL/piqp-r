...
\mathbb{S}_+^n`$, $`A \in \mathbb{R}^{p \times n}`$,
$`G \in \mathbb{R}^{m
\times n}`$, and vectors $`c \in \mathbb{R}^n`$, $`b \in \mathbb{R}^p`$,
$`h_l
...
elements of $`x_l`$ or $`x_u`$ are $`-\infty`$ or $`\infty`$,
respectively. The inequality constraints now support double-sided bounds
$`h_l \leq Gx \leq h_u`$. For one-sided inequalities $`Gx \leq h`$,
...
interface if any of the supplied inputs ($`A`$, $`P`$, or $`G`$) is a
